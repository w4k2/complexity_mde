...
# best
# transfer_names = ['model_synth15260True_di_imgnet']
# worst
transfer_names = ['model_synth1960True_di_imgnet']
...
# scores = np.load("results/transfer/di_bac_synth15260True_imgnet.npy")
# transrates = np.load("results/transfer/di_transrates_synth15260True_imgnet.npy")
...
for data_id, dataset_name in enumerate(tqdm(datasets)):
    ...
    for fold_id, (train_index, test_index) in enumerate(tqdm(rskf.split(X, y), leave=False, desc=f"{data_id}", total=10)):
        ...
        for transfer_id, transfer in enumerate(transfer_names):
            ...
            model = torch.load("models/%s.pt" % transfer, weights_only=False)
            ...
            # best
            # np.save("results/transfer/di_bac_synth15260True_imgnet", scores)
            # np.save("results/transfer/di_transrates_synth15260True_imgnet", transrates)
            # worst
            np.save("results/transfer/di_bac_synth1960True_imgnet", scores)
            np.save("results/transfer/di_transrates_synth1960True_imgnet", transrates)