...
class STML_RGB(TransformerMixin):
    ...
    def fit(self, X, y):
        ...
        self.anova_ = np.nan_to_num(SelectKBest(k='all').fit(self.X_, self.y_).scores_)
        cmap = mpl.colormaps['gist_rainbow_r']
        self.mmanova_ = self.map_value(self.anova_, np.min(self.anova_), np.max(self.anova_), 0, 1)
        # self.colors_ = cmap(self.mmanova_)

        self.colors_ = [self.hex_to_rgb(mpl.colors.to_hex(cmap(i))) for i in self.mmanova_]
...
class STML_RGB2(TransformerMixin):
    def __init__(self, size=(224, 224), verbose=False, n_cols=None):
        self.size_ = size
        self.verbose_ = verbose
        self.n_cols_ = n_cols
    ...
    def fit(self, X, y):
        ...
        self.anova_ = mms.fit_transform(self.anova_.reshape(-1, 1)).reshape(1, -1).squeeze().astype(int)

        self.mean_ = np.mean(self.X_, axis=0)
        self.mean_ = mms.fit_transform(self.mean_.reshape(-1, 1)).reshape(1, -1).squeeze().astype(int)

        self.std_ = np.std(self.X_, axis=0)
        self.std_ = mms.fit_transform(self.std_.reshape(-1, 1)).reshape(1, -1).squeeze().astype(int)

        self.colors_ = []
        for i in range(self.X_.shape[1]):
            self.colors_.append([self.mean_[i], self.std_[i], self.anova_[i]])
