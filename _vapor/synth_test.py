...
for data_id, dataset_name in enumerate(tqdm(datasets)):
    ...
    for fold_id, (train_index, test_index) in enumerate(tqdm(rskf.split(X, y), leave=False, desc=f"{data_id}", total=10)):
        ...
        model = torch.load("models/model_%s_synth.pt" % dataset_name, weights_only=False)
        ...
        np.save("results/transfer/synth_test_scores", scores)
        np.save("results/transfer/synth_test_transrates", transrates)