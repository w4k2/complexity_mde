...
mean_hscore = np.mean(hscore, axis=1)

# print(mean_scores)
# print(mean_transrates)
# print(mean_hscore)
...
for i in range(len(transfer_names)):
    if i == 0 or i == 1:
        ...
        _ = np.mean(mean_hscore[:, i])
        mean_plot_hscore.append(_)
    else:
        ...
        mean_plot_transrates.append(_)
        _ = np.mean(mean_hscore[calc_idx, i])
        mean_plot_hscore.append(_)

mean_plot_scores = np.array(mean_plot_scores)
print(mean_plot_scores)
mean_plot_transrates = np.array(mean_plot_transrates)
print(mean_plot_transrates)
mean_plot_hscore = np.array(mean_plot_hscore)
print(mean_plot_hscore)
...
plt.tight_layout()
plt.savefig("figures2/complexity/v2_f_regression.png")
plt.savefig("figures2/complexity/v2_f_regression.eps")
# plt.savefig("figures2/complexity/v2_f_regression_wo_imgnet.png")
# plt.savefig("figures2/complexity/v2_f_regression_wo_imgnet.eps")
matplotlib.rcParams.update({'font.size': 16, "font.family" : "monospace"})
...
for data_id, data_name in enumerate(dataset_names):
    ...
    plt.tight_layout()
    plt.savefig("figures2/transfer2/di_transrate_%s" % data_name, dpi=200)
    plt.close()


"""
For each dataset - H-Score
"""
for data_id, data_name in enumerate(dataset_names):
    ...
    for transfer_id, transfer in enumerate(transfer_names):
        if transfer != data_name:
            transfer
            ax.scatter(mean_scores[data_id][transfer_id], mean_hscore[data_id][transfer_id], label=transfer, color=colors[transfer_id])
            ax.text(mean_scores[data_id][transfer_id]+.001, mean_hscore[data_id][transfer_id]-.01, s=transfer, fontsize=8)
    ...
    plt.tight_layout()
    plt.savefig("figures2/transfer2/di_hscore_%s" % data_name, dpi=200)
    plt.close()