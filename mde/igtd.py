...
def table_to_image_array(norm_d, scale, fea_dist_method, image_dist_method, max_step, val_step,
                   error, switch_t=0, min_gain=0.00001):
    '''
    This function converts tabular data into images using the IGTD algorithm. 
...
def IGTD_npy(X, max_step = 10000, val_step = 100, fea_dist_method = 'Euclidean', image_dist_method = 'Euclidean', error = 'abs'):
    """
    max_step -- The maximum number of iterations to run the IGTD algorithm, if it does not converge.
    val_step -- The number of iterations for 
    
    determining algorithm convergence. If the error reduction rate is smaller than a pre-set threshold for val_step itertions, the algorithm converges.
    """
