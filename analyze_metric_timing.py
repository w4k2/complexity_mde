...
l2_mean, l2_std = get_mean_std("complexity_l2", "raw_tabular")
density_mean, density_std = get_mean_std("complexity_density", "raw_tabular")
...
complexity_l2_ds = dataset_mean("complexity_l2", "raw_tabular")
complexity_density_ds = dataset_mean("complexity_density", "raw_tabular")

df2 = pd.DataFrame({
    "l2": complexity_l2_ds,
    "density": complexity_density_ds,
    "hscore_full": hs_full,
    "transrate_full": tr_full,
}).reset_index()

plot_df2 = df2.sort_values("l2", ascending=False).reset_index(drop=True)
...
ax.bar(x - 1.5*w, plot_df2["l2"], width=w, label="L2")
ax.bar(x - 0.5*w, plot_df2["density"], width=w, label="Density")
